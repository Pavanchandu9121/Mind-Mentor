...
// POST /screening-result — submit live screening result (called internally or by Gateway)
router.post('/screening-result', protect, async (req, res) => {
  try {
    const { 
      riskLevel, severity, confidence, 
      phq9Score, gad7Score, breakdown, analysis, transcribedText, recommendations
    } = req.body;

    const assessment = await Assessment.create({
      user: req.user._id,
      assessmentType: 'LiveScreening',
